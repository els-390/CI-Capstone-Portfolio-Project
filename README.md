...
Python testing was carried out using a combination of the Code Institute Python Linter, the `flake8 .` terminal command and `pycodestyle` terminal program to check for Python PEP8 compliance. Python issues were fixed either directly when checking or using the Python Formatter in VS Code. I excluded the `.vscode` and `migration` folders as the former is to do with the editor and the latter is generated by Django.
